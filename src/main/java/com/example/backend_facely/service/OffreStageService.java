...
public class OffreStageService {
    ...
    public OffreStageService(OffreStageRepository repository, EntrepriseRepository entrepriseRepository) { this.repository = repository; this.entrepriseRepository = entrepriseRepository; }
    ...
    public OffreStage create(OffreStage dto) {
        validateDates(dto);
        var entreprise = entrepriseRepository.findById(dto.getEntrepriseId()).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Entreprise introuvable"));
        if (entreprise.getStatutValidation() != StatutValidation.VALIDEE) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "L entreprise doit être validée par un administrateur avant de publier une offre");
